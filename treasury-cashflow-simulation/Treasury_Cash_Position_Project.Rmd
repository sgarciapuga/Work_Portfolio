...
```{Python}
from paths import get_balances_path, get_movements_path

print("Balances stored in:", get_balances_path())
print("Movements stored in:", get_movements_path())
```
...
df = pd.read_sql("SELECT * FROM balances ORDER BY date DESC LIMIT 10", conn)
conn.close()
...
df = pd.read_sql("SELECT * FROM movements ORDER BY date DESC LIMIT 10", conn)
conn.close()
